 ...
 */
Binder.prototype.compile = function (subject) {
  // compile statements into context parser rules
  var syntax = this.context._syntax;
  [].push.apply(
    this.context.parsers,
    this.statements.map(_createParser.bind(null, syntax))
  );
  this.statements = [];
  return s.scan(this.context, subject, this.compileTimeout);
};
  ...
  try {
    for (i = 0; i < commands.length; i++) {
      ...
      if (_last(commands) === cmnd) {
        ...
        cond.add(after);
        cond.defaultParams = [undefined, undefined].concat(args);
      }
    }
  } catch (er) {
    cond = parse = null;
