...
Context.prototype.createParserExpr = function() {
  ...
  var syntax = this._syntax,
      name;
  // Conditions
  function createCondition (condDef) {
    return function () {
  ...
  for (name in syntax.conditions) {
    expr[name] = createCondition(syntax.conditions[name]);
  }
  // Parsers
  // function createParser (pDef) {
  //   return function () {
  ...
  //   expr[name] = createCondition(syntax.parsers[name]);
  // }
  return expr;
};
