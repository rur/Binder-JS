...
function __createCondStatement (expr, condDef) {
  return function () {
    ...
    expr.when(function (p,c) {
      return condDef.func.apply(null, [p, c].concat(params.args));
    });
...
function __createParseStatement (expr, pDef) {
  return function () {
    var params = __parseArgs(arguments);
    expr.parse(function (pth, cxt) {
      var promise = when.lift(pDef.func).apply(null, [pth, cxt].concat(params.args));

      if (params.after) {
        promise = promise.then(function(data) {
          return when.lift(params.after).apply(null, [data, cxt]);
        });
      }
      return promise;
    });
