...
Promise.prototype.then = function (onResolve, onReject) {
  ...
  if (typeof onReject === "function") {
    this.hasCatcher = true;
  }
  this._thens.push({
    resolve: ctrl.handle(onResolve),
  ...
  return function () {
    ...
    try {
      if (typeof func === "function") {
        returned = func.apply(ctrl, arguments);
      }
    } catch (er) {
      if (ctrl.promise.hasCatcher) {
        ctrl.reject(er);
      } else {
        throw er; // re-throw
      }
    ...
    if (returned !== ctrl.promise && !ctrl.promise.isCompleted) {
...
function callAsync(fnc) {
  ...
  try {
    ctrl.handle(fnc).apply(null, args);
  } catch (err) {
    ctrl.reject(err);
  }
  return ctrl.promise;
